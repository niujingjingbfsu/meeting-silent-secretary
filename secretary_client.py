...
class MeetingSecretaryClient:
    ...
    async def _run_async_task(self, task: str, task_label: str = None, announce_done: bool = True,
        ...
        instr = (
            ...
            f"【重要】你在这里代表的是这个正在参会的 bot 本身，不代表任何个人——你调用的所有 "
            f"lark-cli 命令都必须显式加 `--as bot`，不要依赖默认身份/不要用你自己的个人身份，"
            f"哪怕当前环境里同时绑了某个人的个人身份、默认身份不是 bot 也一样，必须显式指定。\n\n"
            f"请完成它（可使用你的记忆和 lark 等技能）。完成后：\n"
            f"用 lark-cli 的 `vc +meeting-message-send --as bot --meeting-id {meeting_id} "
            f"--msg-type text --text \"<结果>\"` 把结果作为一条会中弹幕发出去（这是唯一的主要出口，"
            f"务必发，结果简洁、面向人读；这条弹幕所有会议参与者都能看到，不要提脚本文件名/路径/内部"
            f"实现细节这类技术信息，只说人话结论）。\n"
