...
> - **接下来会怎么装**：装前环境探测 → 拉取代码+装依赖 → 检查/自动装 lark-cli → 六层自检（本地环境/lark-cli本体/lark-cli具体命令有没被裁剪/执行大脑/配置文件/飞书会议权限）→ 生成安装报告 → 唤醒词有兜底默认值「小助手」不用你填也能用，想自定义再告诉我 → 找一场真实会议做冒烟测试确认真的能用。
...
唤醒词通过 `bot_name`（兜底默认值「小助手」，不填也能用）/`bot_name_alt`（可选，自定义唤醒词）两个实例属性配置，装到另一个 bot 上不改也能直接用，想自定义只需要改这两个值，不用碰 prompt 正文。2026-08-13 修过一个真实 bug：`bot_name_alt` 曾经只是个存在于 `__init__` 里的死属性，从没真正拼进过判断层的 prompt——填了也没用，现在改成两个名字都会被拼进去。
...
见 [`README.md`](./README.md)：复制本目录 → 复制 `config_silent.yaml`（唤醒词有兜底默认值，不改也能直接用）→ 跑 `python3 onboarding_check.py` 自检本地环境/lark-cli/执行层大脑/配置文件（VC scope 这一层没法零副作用预检，脚本会明确标 ❓，靠下一步冒烟测试验证）→ 冒烟测试（找一场真实进行中的会议，说出唤醒词+一件具体的事，确认收到"收到任务N"弹幕 + 完成后带 ✅/❌ 的弹幕，报错跟着 hint 走）→ 测试完记得 kill 掉进程，不要留残留常驻进程占着已结束的会议轮询。
...
2. **还需要用户做什么**：唤醒词有兜底默认值不强制要求填，但飞书应用有没有开通对应权限这块必须提。
...
## 装完之后：必须给用户讲清楚怎么用（不能装完就结束）

安装报告只解决"装得对不对"，用户还需要知道"日常怎么用"。把下面这段实操指南**转述给用户**（可以改写措辞，但内容不能少）：

> **怎么让它开始工作**：
> - 手动模式（默认）：找一场真实会议，跑 `python3 secretary_transcript_main.py --meeting-no <会议号>`，它会自动加入并发一条开场白弹幕。
> - 自动模式（可选，见「核心机制五」）：把 bot 拉进会议，会自动触发入会，不用手动敲命令。
>
> **进会之后怎么跟它交互**：
> 1. 平时只是安静地听，不会说话、不会主动打断，这是设计上就是这样，不是故障。
> 2. 想让它干活，直接在会里说话，带上唤醒词（默认"小助手"，比如"小助手，帮我查一下上季度的销售数据"）——不喊唤醒词它不会响应。
> 3. 喊到它之后，它会先发一条"收到任务N：<你说的原话>"的弹幕确认收到，然后在后台去执行。
> 4. 干完之后会再发一条结果弹幕，**开头带 ✅ 或 ❌**——✅ 表示真的做成了；❌ 表示没做成（权限不够/信息缺失/任何原因），弹幕里会说清楚原因，不会假装做成了。
>
> **能让它干什么**：凡是接的那个"大脑"（默认 Claude）能做到的事都行——查资料、写文档摘要、建表格、查聊天记录、总结讨论内容等；具体能力上限取决于换了哪个执行层，不是这个 Skill 本身画的边界。
>
> **不能干什么**：不会语音说话（永远不会，设计如此）；不含实时Slides投屏/脑暴看板/持续记纪要这类"一直在跑、自动刷新"的常驻功能，只做"听到指令→执行→回结果"这一次性的事；`RECONNECT`（要求重连）目前只会回复"无法自动重连"，不会真的重连。
>
> **如果好像没反应**：先确认① 说的是配置里那个唤醒词（判断层是 LLM 语义判断，不是精确字符串匹配，但完全不沾边的名字肯定不会被认出来）② 进程真的在跑、真的在这场会里（看有没有收到入会时发的开场白弹幕）③ 是不是同一句话里既有闲聊又有指令，被判断层当成"还在讨论"而不是"直接对我说的指令"——见「核心机制一」里"务必分清闲聊和直接指令"那条规则。
