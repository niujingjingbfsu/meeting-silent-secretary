...
description: 会中无声助手（会议秘书）——已打包成独立、自包含、可直接安装的能力包（本目录）：会中全程不语音发言，只默默听（走飞书自己的逐字稿，不接第三方 ASR），被明确叫到名字才执行一次性任务，任务结果发会中弹幕，还支持6个可开关的持续性技能。当用户说"给XX也装一个无声助手/会议秘书能力"、"把这套装成Skill给别的Agent用"、"这个无声助手怎么移植"时使用。
---

# 会中无声助手 (meeting-silent-secretary)

会中一种角色：全程**不语音发言**、不打断会议，只在后台听，被人**明确叫到名字**时才响应，静默执行一件事，结果通过会中弹幕发出来；支持6个可开关的持续性技能（脑暴看板/口述建原型/会议纪要/实时会议Slides/分享要点卡片/持续答弹幕），开启后常驻循环、随讨论持续刷新内容、产出可访问的 HTML 页面。

本目录是一个**独立、自包含的能力包**（不依赖 `lark-voice-agent` 主项目），可以直接复制走使用——见 [`README.md`](./README.md) 的安装/运行步骤。核心实现在 `secretary_client.py`（`MeetingSecretaryClient` 类），入口脚本是 `secretary_transcript_main.py`。
...
（源项目里还有一条"豆包混音耳朵+实时语音"路径，那是给需要真人语音交互的主持人角色用的，跟本能力包无关，没有包含在这里。）
...
- 6 个持续性技能的开关标签（`BARRAGE_ANSWER_ON/OFF`、`BOARD_ON/OFF`、`PROTOTYPE_ON/OFF`、`MINUTES_ON/OFF`、`SLIDES_ON/OFF`、`TALKPOINTS_ON/OFF`，以及批量标签 `SKILLS_ON:`/`SKILLS_OFF:`）——判断层 prompt 里特别强调一条易错点：一次性任务（"帮我写份今天的会议纪要"）不等于开启持续技能（"从现在开始一直跟着记纪要"），判断标准是这句话里有没有"持续/一直/跟着"这类长期跟随的词，拿不准默认 `DO`，不要轻易开持续技能。
...
`_meeting_chat_loop` 轮询 `vc +meeting-events`，同一条轮询处理三种事件：`transcript_received`（逐字稿，带真实说话人）、会中弹幕（`chat_received_items`）、投屏文档（`magic_share_started`）。拼出来的 `[姓名]: 原话` 直接喂进 `_feed_host_transcript`，下游 DO/结论/技能开关逻辑天然就能拿到"是谁说的"这个信息。
...
`barrage_answer`（持续答弹幕）/ `board`（脑暴看板）/ `prototype`（口述建原型）/ `minutes`（会议纪要）/ `slides`（实时会议Slides）/ `talkpoints`（分享要点卡片）——统一走 `_SKILL_KEYS` 开关框架（`_activate_skill`/`_deactivate_skill`），每个技能各自一对 `_xxx_loop`（常驻监测，"内容驱动+防抖"或固定 poll 间隔）+ `_xxx_regen_async`（真正调用带工具的子 Claude 生成内容）。`slides`/`talkpoints` 用的样式模板在 `templates/` 目录下，全量重生成不做增量 patch。

产出物发布走 `_publish_page`：配置了 `remote_publish_target`（形如 `user@host:/path`）才会真的 scp 推送到远程服务器；配置了 `public_base_url` 才会在弹幕里给出可点击链接；两者都没配置时只在本地保存文件，弹幕里说明"已保存本地"。

**已知限制**：议程跟进技能未实现（不在判断层标签列表里）；不支持切换成会说话的语音人设（那需要接入完全独立的实时语音/TTS系统，本能力包不含）。
...
- Claude CLI（`claude` 命令，带 `--dangerously-skip-permissions`）——判断层常驻会话 + 每个 DO 任务/持续技能生成的子进程都靠它。
